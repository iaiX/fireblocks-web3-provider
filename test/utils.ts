...
export function getFireblocksProviderForTesting(extraConfiguration?: any) {
  if (
    ...
    throw new Error(
      "Environment variables FIREBLOCKS_API_PRIVATE_KEY_PATH, FIREBLOCKS_API_KEY, FIREBLOCKS_VAULT_ACCOUNT_IDS must be set"
    );
  }
...
export function getEthersFireblocksProviderForTesting(
  ...
  return new ethers.providers.Web3Provider(
    getFireblocksProviderForTesting(extraConfiguration)
  );
}
