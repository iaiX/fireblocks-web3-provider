...
if (process.env.FIREBLOCKS_GAS_TANK_VAULT_ID === undefined) {
  throw new Error(
    "FIREBLOCKS_GAS_TANK_VAULT_ID environment variable must be set for gasless tests"
  );
}
...
const provider = getEthersFireblocksProviderForTesting({
  gaslessGasTankVaultId: parseInt(process.env.FIREBLOCKS_GAS_TANK_VAULT_ID!),
});
...
describe("Gasless: should be able to send a transaction using a gasless relay server", function () {
  ...
  it("Mint", async function () {
    ...
    const GaslessToken = new ethers.Contract(
      gaslessTokenAddress,
      GaslessTokenAbi,
      signer
    );
