...
async function getFirstAddressWithBalance() {
  ...
  throw new Error(
    `No vault has balance greater than ${transferAmount.toString()}`
  );
}
...
describe("Ethers: Should be able to transfer ETH", function () {
  ...
  it("Transfer", async function () {
    ...
    const toAddress = addresses.find(
      (x) => x.toLowerCase() != firstAddressWithBalance
    );
    ...
    expect(
      toAddressEndingBalance.eq(toAddressStartingBalance.sub(transferAmount))
    );
  });
});
