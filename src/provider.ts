...
export class FireblocksWeb3Provider extends HttpProvider {
  ...
  constructor(config: FireblocksProviderConfig) {
    ...
    this.assetAndChainIdPopulatedPromise = promiseToFunction(async () => {
      if (!this.chainId) return this.populateAssetAndChainId();
    });
  ...
  private async populateGaslessGasTankAddress(): Promise<void> {
    ...
    const depositAddresses = await this.fireblocksApiClient.getDepositAddresses(
  ...
  private async populateAccounts() {
    ...
    for (const vaultAccountId of this.vaultAccountIds) {
      ...
      try {
        depositAddresses = await this.fireblocksApiClient.getDepositAddresses(
    ...
    (async () => {
      ...
      try {
        switch (payload.method) {
          ...
          case "personal_sign":
          case "eth_sign":
            result = await this.createPersonalSign(
              payload.params[1],
              payload.params[0],
